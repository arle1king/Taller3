...
public class Aerolinea
{
    ...
    {
        // TODO implementar
    	Collection<Tiquete> tiquetes = new LinkedList<>();
        for (Vuelo vuelo : vuelos) {
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (fecha == null || fecha.trim().isEmpty() ||
            codigoRuta == null || codigoRuta.trim().isEmpty() ||
            nombreAvion == null || nombreAvion.trim().isEmpty()) {
            throw new IllegalArgumentException("Datos del vuelo no pueden ser nulos o vacíos");
        }

        // Verificar disponibilidad del avión
        Avion avion = encontrarAvionPorNombre(nombreAvion);
        if (avion == null) {
            throw new Exception("El avión especificado no existe");
        }

        // Crear el nuevo vuelo y agregarlo a la lista de vuelos programados
        Vuelo nuevoVuelo = new Vuelo(null, fecha, avion);
        vuelos.add(nuevoVuelo);
    }

    // Buscar un avión por su nombre
    private Avion encontrarAvionPorNombre(String nombreAvion) {
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (vuelo.getEspacioDisponible() < cantidad) {
            throw new VueloSobrevendidoException(vuelo);
        }

        // Determinar la tarifa según la fecha
        int tarifa =CalculadoraTarifas.calcularTarifa(vuelo,cliente);

        // Crear los tiquetes y asociarlos al cliente y al vuelo
    ...
    public String consultarSaldoPendienteCliente( String identificadorCliente ) throws Exception
    {
        ...
        int numero=cliente.calcularValorTotalTiquetes();
        return ""+numero; 
    }
